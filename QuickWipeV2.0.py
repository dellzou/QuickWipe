...
class DiskWiper:
    def __init__(self):
        ...
        # SSD块大小配置（单位：字节）
        self.ssd_block_sizes = {
            'TLC': 16 * 1024,  # 16KB块（常见TLC SSD）
            'MLC': 8 * 1024,  # 8KB块（MLC SSD）
            'SLC': 4 * 1024,  # 4KB块（SLC SSD）
            'QLC': 32 * 1024,  # 32KB块（QLC SSD）
            'UNKNOWN': 16 * 1024  # 默认16KB
        }
                ...
                if disk.Index == disk_number:
                    ...
                    if "ssd" in model or "solid state" in model:
                        ...
                        if "tlc" in model:
                            ...
                            nand_type = "QLC"
                        elif "v-nand" in model or "v nand" in model:
                            nand_type = "TLC"  # 三星V-NAND通常是TLC
                    ...
                    return disk_type, interface_type, nand_type
    ...
    def get_disk_info(self, disk_identifier: str) -> Tuple[str, str, str, str]:
        """
        获取磁盘信息并返回建议的填充率
        返回: (disk_type, interface_type, nand_type, rate_key)
        """
        if self.system == "Linux":
            ...
            disk_type, interface_type, nand_type = self.get_disk_info_windows(disk_number)
        else:
        ...
        return disk_type, interface_type, nand_type, rate_key
    ...
    def wipe_disk_windows(self, disk_number: int, fill_rate: float, pattern: int = 0) -> bool:
        ...
        try:
            ...
            for partition in partitions:
                ...
                # 使用不同的擦除方法
                print("选择擦除方法:")
                print("1. cipher安全擦除 (3次覆盖)")
                print("2. format格式化 (1-3次覆盖)")
                print("3. 创建大文件填充")
                print("4. SSD优化擦除 (小文件触发垃圾回收)")
                method = input("请选择 (1-4): ").strip()
                ...
                if method == "1":
                    ...
                    # SSD优化擦除 - 小文件触发垃圾回收
                    if not self.wipe_ssd_optimized(partition['letter'], partition_fill_bytes, disk_number):
                        success = False
                else:
                    print("无效选择，跳过此分区")
...
"""
            ...
            # 使用优化擦除方法
            print("使用SSD优化擦除方法...")
            disk_info = self.get_disk_info_windows(disk_number)
            if disk_info[0] == "SSD":
                # 对SSD使用优化擦除
                partition_info = {
                    'letter': 'X:',
                    'size_bytes': 10 * 1024 ** 3,  # 估计10GB
                    'size_gb': 10,
                    'free_space': 9 * 1024 ** 3
                }
                success = self.wipe_ssd_optimized('X:', int(10 * 1024 ** 3 * fill_rate), disk_number)
            else:
                # 对HDD使用cipher
                cipher_result = subprocess.run('cipher /w:X:', shell=True, capture_output=True, text=True)
                success = cipher_result.returncode == 0
...
"""
    ...
    def wipe_ssd_optimized(self, drive_letter: str, fill_bytes: int, disk_number: int) -> bool:
        """
        SSD优化擦除 - 通过创建大量小文件触发垃圾回收
        基于用户的想法：写入小文件占用SSD物理块，触发控制器擦除
        """
        try:
            print(f"使用SSD优化擦除方法处理驱动器 {drive_letter}...")
            print("原理: 创建大量小文件占用SSD物理块，触发控制器的垃圾回收机制")

            # 获取SSD信息
            disk_type, interface_type, nand_type = self.get_disk_info_windows(disk_number)

            if disk_type != "SSD":
                print("警告: 此方法专为SSD设计，当前磁盘可能不是SSD")
                confirm = input("是否继续? (y/N): ").strip().lower()
                if confirm != 'y':
                    return False

            # 确定SSD块大小
            block_size = self.ssd_block_sizes.get(nand_type, 16 * 1024)
            print(f"检测到SSD类型: {nand_type}, 使用块大小: {block_size / 1024}KB")

            # 计算需要创建的文件数量
            # 每个文件1KB，确保占用完整的SSD块
            file_size = 1024  # 1KB
            files_per_block = block_size // file_size

            # 计算需要占用的块数
            blocks_needed = math.ceil(fill_bytes / block_size)
            total_files_needed = blocks_needed * files_per_block

            print(f"需要填充: {fill_bytes / (1024 ** 3):.2f} GB")
            print(f"SSD块大小: {block_size / 1024}KB")
            print(f"需要占用块数: {blocks_needed}")
            print(f"需要创建文件数: {total_files_needed}")
            print(f"每个文件大小: {file_size}字节")

            # 创建临时目录
            temp_dir = os.path.join(drive_letter, "SSD_WIPE_TEMP")
            if os.path.exists(temp_dir):
                shutil.rmtree(temp_dir)
            os.makedirs(temp_dir)

            print("开始创建小文件触发SSD垃圾回收...")

            # 创建多个子目录分散文件
            subdirs = 10
            files_per_subdir = total_files_needed // subdirs + 1

            for subdir in range(subdirs):
                subdir_path = os.path.join(temp_dir, f"dir_{subdir:03d}")
                os.makedirs(subdir_path)

                for file_idx in range(files_per_subdir):
                    file_num = subdir * files_per_subdir + file_idx
                    if file_num >= total_files_needed:
                        break

                    # 生成随机文件名和内容
                    filename = os.path.join(subdir_path, f"file_{file_num:08d}.tmp")
                    ...
                    # 显示进度
                    if file_num % 1000 == 0:
                        progress = (file_num + 1) / total_files_needed * 100
                        print(f"进度: {progress:.1f}%, 已创建文件: {file_num + 1}/{total_files_needed}", end='\r')

            print(f"\n文件创建完成，共创建 {total_files_needed} 个文件")
            print("等待SSD控制器处理垃圾回收...")
            time.sleep(5)  # 给SSD控制器一些时间
            ...
            # 发送TRIM命令（通过优化驱动器）
            print("发送TRIM命令通知SSD回收空间...")
            try:
                # 使用defrag命令发送TRIM
                result = subprocess.run(f'defrag {drive_letter} /O', shell=True, capture_output=True, text=True)
                if result.returncode == 0:
                    print("TRIM命令发送成功")
                else:
                    print(f"TRIM命令发送失败: {result.stderr}")
            except Exception as e:
                print(f"发送TRIM命令时出错: {e}")

            # 可选：填充随机数据以覆盖可能残留的数据
            confirm = input("是否填充随机数据覆盖可能残留的元数据? (y/N): ").strip().lower()
            if confirm == 'y':
                print("填充随机数据覆盖元数据...")
                self.fill_random_metadata(drive_letter, block_size, 100)  # 填充100个块

            print(f"驱动器 {drive_letter} SSD优化擦除完成!")
            print("注意: SSD的垃圾回收是后台进行的，完全擦除可能需要一些时间")
            return True
    ...
    def fill_random_metadata(self, drive_letter: str, block_size: int, num_blocks: int) -> bool:
        """填充随机数据覆盖可能残留的元数据"""
        try:
            temp_dir = os.path.join(drive_letter, "METADATA_WIPE")
            if os.path.exists(temp_dir):
                shutil.rmtree(temp_dir)
            os.makedirs(temp_dir)

            print(f"填充 {num_blocks} 个块的随机数据覆盖元数据...")

            for i in range(num_blocks):
                filename = os.path.join(temp_dir, f"meta_{i:06d}.bin")
                # 创建随机数据
                random_data = bytes([random.randint(0, 255) for _ in range(block_size)])

                with open(filename, 'wb') as f:
                    f.write(random_data)

                if i % 10 == 0:
                    print(f"进度: {i + 1}/{num_blocks}", end='\r')
...
def interactive_mode():
    ...
    while True:
        ...
        if choice == "1":
            ...
            print("\n正在分析磁盘信息...")
            try:
                disk_type, interface_type, nand_type, rate_key = wiper.get_disk_info(disk_input)

                print(f"\n磁盘信息:")
                print(f"  类型: {disk_type}")
                print(f"  接口: {interface_type}")
                if disk_type == "SSD":
                    print(f"  NAND类型: {nand_type}")
...
def main():
    ...
    # 获取磁盘信息
    print("正在分析磁盘信息...")
    disk_type, interface_type, nand_type, rate_key = wiper.get_disk_info(args.disk)
